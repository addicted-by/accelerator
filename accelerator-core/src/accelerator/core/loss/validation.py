...
class InputValidator:
    ...
    def validate(
        ...
        try:
            ...
            raise ValidationError(
                ...
                context={"validation_level": config.level.value}
            ) from e
    ...
    def _validate_dict_inputs(predictions: Dict, labels: Dict, config: ValidationConfig) -> None:
        ...
        for key, value in predictions.items():
            if not isinstance(value, torch.Tensor):
                raise ValidationError(
                    f"All prediction values must be torch.Tensor, got {type(value)} for key '{key}'"
                )
        
        for key, value in labels.items():
            if not isinstance(value, torch.Tensor):
                raise ValidationError(
                    f"All label values must be torch.Tensor, got {type(value)} for key '{key}'"
                )
        ...
        if pred_keys != label_keys:
            ...
            raise ValidationError(
                "Predictions and labels must have matching keys",
                context={"mismatched_keys": list(mismatched)}
            )
    ...
    @staticmethod
    def _validate_tensor_properties(pred: torch.Tensor, label: torch.Tensor, config: ValidationConfig, key: str = None) -> None:
        ...
        if config.check_dtype and pred.dtype != label.dtype:
            logger.warning(
                f"Predictions and labels have different dtypes: {pred.dtype} vs {label.dtype}"
            )