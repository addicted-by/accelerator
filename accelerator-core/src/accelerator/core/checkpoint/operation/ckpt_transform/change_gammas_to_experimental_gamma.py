...
def change_gammas_to_experimental_gamma(model, ckpt_model_dict):
    ...
    x_frames_forward = x_forward.reshape(1, 1, 1, -1).repeat(
        1, model.gamma._gamma_forward.gamma_frames.num_channels, 1, 1
    )
    if hasattr(model.gamma._gamma_forward, "gamma_meta"):
        x_meta_forward = x_forward.reshape(1, 1, 1, -1).repeat(
            1, model.gamma._gamma_forward.gamma_meta.num_channels, 1, 1
        )
    else:
    ...
    # model.gamma._gamma_forward.gamma_frames.deltas = torch.nn.Parameter(model.gamma._gamma_forward.gamma_frames.get_deltas(y_frames_forward.squeeze()), requires_grad=True)

    # model.gamma._gamma_forward.gamma_meta.deltas = torch.nn.Parameter(model.gamma._gamma_forward.gamma_meta.get_deltas(y_meta_forward.squeeze()), requires_grad=True)

    # model.gamma._gamma_inverse.gamma_frames.deltas = torch.nn.Parameter(model.gamma._gamma_inverse.gamma_frames.get_deltas(y_frames_inverse.squeeze()), requires_grad=True)
    ...
    ###############################################################
    ckpt_model_dict["gamma._gamma_forward.gamma_frames.deltas"] = model.gamma._gamma_forward.gamma_frames.get_deltas(
        y_frames_forward.squeeze()
    )
    if model.gamma._gamma_forward.gamma_frames.trainable_endpoints:
        ckpt_model_dict["gamma._gamma_forward.gamma_frames.start_points"] = torch.zeros(
            model.gamma._gamma_forward.gamma_frames.num_channels, 1
        )
    if hasattr(model.gamma._gamma_forward, "gamma_meta"):
        log.info("!" * 100)
        ...
        ckpt_model_dict["gamma._gamma_forward.gamma_meta.deltas"] = model.gamma._gamma_forward.gamma_meta.get_deltas(
            y_meta_forward.squeeze()
        )
        if model.gamma._gamma_forward.gamma_meta.trainable_endpoints:
            ckpt_model_dict["gamma._gamma_forward.gamma_meta.start_points"] = torch.zeros(
                model.gamma._gamma_forward.gamma_meta.num_channels, 1
            )
    ...
    log.info("GAMMA INVERSE FRAMES BEFORE:")
    log.info(model.gamma._gamma_inverse.gamma_frames.deltas)
    ...
    if hasattr(model.gamma._gamma_forward, "gamma_meta"):
        ...
        log.info(model.gamma._gamma_forward.gamma_meta.deltas)
    log.info("GAMMA INVERSE FRAMES AFTER:")
    log.info(model.gamma._gamma_inverse.gamma_frames.deltas)
