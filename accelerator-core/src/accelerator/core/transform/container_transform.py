...
class ContainerCondition:
    ...
    def _evaluate_single_condition(self, context, condition: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Condition evaluation: {actual_value} {operator} {expected_value} = {result}"
            )
            return result
        except Exception as e:
    ...
    def __init__(
        ...
        if isinstance(trans_inputs, dict):
            ...
            if 'args' in trans_inputs or 'kwargs' in trans_inputs:
                self.input_getter = ContainerGetItem(
                    args=trans_inputs.get('args'),
                    kwargs=trans_inputs.get('kwargs')
                )
            else:
            ...
            raise ValueError(
                f"trans_inputs must be a list or dict, got {type(trans_inputs)}"
            )
        ...
        if trans_opt is not None:
            ...
            try:
                ...
                self.transform = transforms_registry.get_or_create_instance(
                    trans_type,
                    trans_opt_copy
                )
    ...
    def __call__(self, context) -> Any:
        ...
        if self.transform:
            try:
                ...
                logger.debug(f"Transform executed successfully")
            except Exception as e:
                logger.error(f"Transform execution failed: {e}")
                raise RuntimeError(
                    f"Transform execution failed with config {self.trans_opt_original}: {e}"
                ) from e
        else:
