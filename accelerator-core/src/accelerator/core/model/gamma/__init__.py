...
class Gamma(torch.nn.Module):
    def __init__(self, model_config):
        ...
        if model_config["use_gamma"]:
            ...
            if model_config["gamma_type"] == 143:
                ...
                if not model_config["experimental_gamma_forward"] and not model_config["equalization_forward"]:
                    self.b1 = torch.nn.Parameter(torch.tensor(init_g4[0]), requires_grad=True)
                    self.c1 = torch.nn.Parameter(torch.tensor(init_g4[1]), requires_grad=True)
                    self.d1 = torch.nn.Parameter(torch.tensor(init_g4[2]), requires_grad=True)
                    self.e1 = torch.nn.Parameter(torch.tensor(init_g4[3]), requires_grad=True)
                    self._gamma_forward = self._gamma_forward_polynomial
                if not model_config["experimental_gamma_inverse"]:
                    self.c2 = torch.nn.Parameter(torch.tensor(init_d3[0]), requires_grad=True)
                    self.d2 = torch.nn.Parameter(torch.tensor(init_d3[1]), requires_grad=True)
                    self.e2 = torch.nn.Parameter(torch.tensor(init_d3[2]), requires_grad=True)
        ...
        if model_config["experimental_gamma_forward"] and not model_config["equalization_forward"]:
            self._gamma_forward = ImprovedGammaJDD(
                resolution=model_config["experimental_gamma_forward_resolution"],
        ...
        if model_config["experimental_gamma_inverse"]:
            self._gamma_inverse = ImprovedGammaJDD(
                resolution=model_config["experimental_gamma_inverse_resolution"],
    ...
    def _calc_out(self, x, b1, c1, d1, e1):
        ...
        # TODO: decide what to do with this (some meta channels are larger than 1)
        out = polynomial_mask * out + (1.0 - polynomial_mask) * x
    ...
    def print_gamma_coefs(self):
        if (
            self.model_config["gamma_type"] == 843
            or self.model_config["gamma_type"] == 943
            or self.model_config["gamma_type"] == 743
        ):
            print(
                f"b1={self.b1_arr}; c1={self.c1_arr}, d1={self.d1_arr}, e1={self.e1_arr}; c2={self.c2_arr}, d2={self.d2_arr}, e2={self.e2_arr}"
            )
    ...
    def _gamma_inverse_splitted_linear_white(self, x):
        ...
        out = dark_mask * dark_part + (1.0 - dark_mask) * light_part
...
class ImprovedGammaJDD(nn.Module):
    ...
    def __init__(
        ...
        self.gamma_frames = ImprovedGamma(
