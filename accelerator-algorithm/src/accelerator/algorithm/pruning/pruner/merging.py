...
class MergingPruner(BasePruner):
    ...
    def __init__(self, **kwargs):
        ...
        if self.imp_fn_name:
            ...
            if isinstance(
                self.imp_fn, (feature_merging.importance.TaylorImportance, feature_merging.importance.HessianImportance)
            ):
                if self.verbose:
    ...
    def _record_pruning(self, group, initial_size: int, new_size: int) -> None:
        ...
        self.pruning_record.append((str(group).replace("\n", ""), initial_size, new_size))
    ...
    def _apply_parametrizations(
        self, model, group, initial_size: int, new_size: int, important_indices: Optional[torch.Tensor]
    ) -> None:
    ...
    def _create_feature_merging(
        self,
        initial_size: int,
        new_size: int,
        dim: int,
        important_indices: Optional[torch.Tensor],
        name: str,
        single_parametrization: Optional[torch.nn.Module],
        device: torch.device,
    ) -> feature_merging.FeatureMerging:
