...
from accelerator.domain.cv.utils import get_gaussian_kernel2d
...
def ssim_c(
    ...
    return SSIM(window_size, reduction, max_val, alpha=0, beta=0, gamma=1)(img1, img2)
...
class SSIM(nn.Module):
    ...
    def forward(  # type: ignore
        ...
        if not img1.shape == img2.shape:
            raise ValueError("img1 and img2 shapes must be the same. Got: {} vs {}"
                             .format(img1.shape, img2.shape))
        if not img1.device == img2.device:
            raise ValueError("img1 and img2 must be in the same device. Got: {} vs {}"
                             .format(img1.device, img2.device))
        if not img1.dtype == img2.dtype:
            raise ValueError("img1 and img2 must be in the same dtype. Got: {} vs {}"
        ...
        if self.alpha == 1 and self.beta == 1 and self.gamma == 1: # to avoid sqrt
            ssim_map = ((2 * mu1_mu2 + self.C1) * (2 * sigma12 + self.C2)) / \
                ((mu1_sq + mu2_sq + self.C1) * (sigma1_sq + sigma2_sq + self.C2))
            
        else:
            sigma1_sq = torch.maximum(sigma1_sq, torch.zeros_like(sigma1_sq))
            sigma2_sq = torch.maximum(sigma2_sq, torch.zeros_like(sigma2_sq))
            
            if self.alpha > 0:
                luminance = (2 * mu1_mu2 + self.C1) / (mu1_sq + mu2_sq + self.C1)
            else:
                luminance = torch.tensor(1.0)
                
            if self.beta > 0:
                contrast = (2 * (sigma1_sq + 1e-9).pow(0.5) * (sigma2_sq + 1e-9).pow(0.5) + self.C2) / (sigma1_sq + sigma2_sq + self.C2)
            else:
                contrast = torch.tensor(1.0)
                
            if self.gamma > 0:
                structure = (2 * sigma12 + self.C2) / (2 * (sigma1_sq + 1e-9).pow(0.5) * (sigma2_sq + 1e-9).pow(0.5) + self.C2)
            else:
                structure = torch.tensor(1.0)

            ssim_map = luminance.pow(self.alpha) * contrast.pow(self.beta) * structure.pow(self.gamma)

        loss = torch.clamp(torch.tensor(1.) - ssim_map, min=0, max=2) / 2.
