...
@registry.register_loss(LossType.CUSTOM, 'GradVarianceLoss')
class GradientVariance(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        gray_output = 0.2989 * net_result[:, 0:1, :, :] + 0.5870 * net_result[:, 1:2, :, :] + 0.1140 * net_result[:, 2:, :, :]
        gray_target = 0.2989 * ground_truth[:, 0:1, :, :] + 0.5870 * ground_truth[:, 1:2, :, :] + 0.1140 * ground_truth[:, 2:, :, :]
        ...
        gradvar_loss = torch.abs(var_target_x - var_output_x) + torch.abs(var_target_y - var_output_y)
