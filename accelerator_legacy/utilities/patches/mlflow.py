...
from mlflow.utils.uri import resolve_uri_if_local, append_to_uri_path
...
def _validate_custom_run_id(run_id, session):
    ...
    if existing_run:
        raise MlflowException(
            f"Run with id={run_id} already exists",
...
def _validate_custom_artifact_location(artifact_location):
    ...
    try:
        ...
        if not resolved_location:
            raise MlflowException(
                f"Invalid artifact_location: '{artifact_location}'",
                INVALID_PARAMETER_VALUE
            )
    except Exception as e:
        raise MlflowException(
            f"Invalid artifact_location: '{artifact_location}'. Error: {str(e)}",
            INVALID_PARAMETER_VALUE
        )
    ...
    with self.ManagedSessionMaker() as session:
        ...
        if custom_artifact_location:
            ...
            artifact_location = append_to_uri_path(
                experiment.artifact_location, run_id, self.ARTIFACTS_FOLDER_NAME
            )
        ...
        from mlflow.entities import RunTag, SourceType, RunStatus
