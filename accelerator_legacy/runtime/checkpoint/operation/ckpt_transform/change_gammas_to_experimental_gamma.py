...
def change_gammas_to_experimental_gamma(model, ckpt_model_dict):
    ...
    x_frames_inverse = x_forward.reshape(1, 1, 1, -1).repeat(1, model.gamma._gamma_inverse.gamma_frames.num_channels, 1, 1)
    ...
    ckpt_model_dict['gamma._gamma_inverse.gamma_frames.deltas'] = model.gamma._gamma_inverse.gamma_frames.get_deltas(y_frames_inverse.squeeze())
    if model.gamma._gamma_inverse.gamma_frames.trainable_endpoints:
        ckpt_model_dict['gamma._gamma_inverse.gamma_frames.start_points'] = torch.zeros(model.gamma._gamma_inverse.gamma_frames.num_channels, 1)
