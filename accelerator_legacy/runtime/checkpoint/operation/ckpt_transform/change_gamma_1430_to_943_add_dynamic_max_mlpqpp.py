...
def change_gamma_1430_to_943_add_dynamic_max_mlpqpp(model, ckpt_model_dict):
    log.info('===change_gamma_1430_to_943 and add_dynamic_max_mlpqpp===')
    ...
    keys = list(ckpt_model_dict.keys())
    for n in keys:
        if 'meta' in n:
            ckpt_model_dict.pop(n, None)
